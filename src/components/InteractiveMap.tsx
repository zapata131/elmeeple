...
export default function InteractiveMap() {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[#F5F0E9]">
      ...
      <div
        className={`absolute top-0 left-0 w-full md:w-96 h-auto md:h-full bg-[#F5F0E9] dark:bg-[#1E1E1E] text-[#3A3A3A] dark:text-[#F5F0E9] z-10 border-b md:border-b-0 md:border-r border-[#3A3A3A]/10 dark:border-[#F5F0E9]/10 shadow-2xl flex flex-col transition-all duration-300 backdrop-blur-md bg-opacity-95 ${
      ...
      >
        ...
        <div className="p-4 md:px-6 md:py-4 flex flex-col gap-3 border-b border-[#3A3A3A]/5 dark:border-[#F5F0E9]/5 text-sm">
              ...
              <button
                ...
                className={`px-3 py-1 text-xs font-bold rounded-full border transition-all duration-150 cursor-pointer whitespace-nowrap ${
                  selectedCategory === cat
                    ...
                    : 'bg-white dark:bg-[#2D2D2D] border-[#3A3A3A]/20 dark:border-[#F5F0E9]/20 text-[#3A3A3A] dark:text-[#F5F0E9] hover:border-[#8367C7]/50 dark:hover:border-[#8367C7]/50'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="flex flex-col gap-1.5 mt-1.5">
            <div className="text-[10px] uppercase tracking-wider text-[#3A3A3A]/50 dark:text-[#F5F0E9]/50 font-black">Distancia máxima</div>
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
              {[
            ...
            <>
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl border transition-all duration-200 flex gap-3 cursor-pointer ${
                    selectedVenue?.id === venue.id
                      ? 'bg-[#8367C7]/10 dark:bg-[#8367C7]/15 border-[#8367C7] shadow-sm'
                      : 'bg-white dark:bg-[#2D2D2D] border-[#3A3A3A]/10 dark:border-[#F5F0E9]/10 hover:border-[#8367C7]/30 hover:shadow-md shadow-sm'
                  }`}
                >
                  {venue.logoUrl && (
                    ...
                    <img
                      src={venue.logoUrl}
                      alt={`Logo ${venue.name}`}
                      className="w-12 h-12 rounded-lg object-cover flex-shrink-0 border border-[#3A3A3A]/5 dark:border-[#F5F0E9]/5"
                    />
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-1">
                      <h3 className="font-bold text-sm text-[#3A3A3A] dark:text-[#F5F0E9] truncate">{venue.name}</h3>
                      {venue.reviews && venue.reviews.length > 0 && (
                        <div className="flex items-center gap-0.5 text-xs text-amber-500 font-black flex-shrink-0">
                          ...
                          <span className="text-[10px] text-[#3A3A3A]/50 dark:text-[#F5F0E9]/50 font-semibold ml-0.5">
                            ({venue.reviews.length})
                          </span>
                        </div>
                      )}
                    ...
                    {searchQuery.trim() !== '' && (
                      <div className="mb-2">
                        {(() => {
                          ...
                          const venueTypes = venue.type ? venue.type.split(',') : []
                          const isCafeOrClub = venueTypes.includes('cafe') || venueTypes.includes('comunidad')
                          const isTienda = venueTypes.includes('tienda')
                          
                          let label = 'Tiene'
                          if (isCafeOrClub && isTienda) {
                            label = 'Ludoteca y Venta'
                          } else if (isCafeOrClub) {
                            label = 'Para jugar'
                          } else if (isTienda) {
                            label = 'En venta'
                          }
                          ...
                          return (
                            <div className="flex items-center gap-1.5 px-2 py-0.5 bg-[#8367C7]/5 dark:bg-[#8367C7]/15 border border-[#8367C7]/15 dark:border-[#8367C7]/25 rounded-md text-[11px] font-bold text-[#8367C7] w-fit">
                              <svg className="w-3 h-3 text-[#8367C7] fill-current" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                                <path d="M256 54.99c-27 0-46.418 14.287-57.633 32.23-10.03 16.047-14.203 34.66-15.017 50.962-30.608 15.135-64.515 30.394-91.815 45.994-14.32 8.183-26.805 16.414-36.203 25.26C45.934 218.28 39 228.24 39 239.99c0 5 2.44 9.075 5.19 12.065 2.754 2.99 6.054 5.312 9.812 7.48 7.515 4.336 16.99 7.95 27.412 11.076 15.483 4.646 32.823 8.1 47.9 9.577-14.996 25.84-34.953 49.574-52.447 72.315C56.65 378.785 39 403.99 39 431.99c0 4-.044 7.123.31 10.26.355 3.137 1.256 7.053 4.41 10.156 3.155 3.104 7.017 3.938 10.163 4.28 3.146.345 6.315.304 10.38.304h111.542c8.097 0 14.026.492 20.125-3.43 6.1-3.92 8.324-9.275 12.67-17.275l.088-.16.08-.166s9.723-19.77 21.324-39.388c5.8-9.808 12.097-19.576 17.574-26.498 2.74-3.46 5.304-6.204 7.15-7.754.564-.472.82-.56 1.184-.76.363.2.62.288 1.184.76 1.846 1.55 4.41 4.294 7.15 7.754 5.477 6.922 11.774 16.69 17.574 26.498 11.6 19.618 21.324 39.387 21.324 39.387l.08.165.088.16c4.346 8 6.55 13.323 12.61 17.254 6.058 3.93 11.974 3.45 19.957 3.45H448c4 0 7.12.043 10.244-.304 3.123-.347 6.998-1.21 10.12-4.332 3.12-3.122 3.984-6.997 4.33-10.12.348-3.122.306-6.244.306-10.244 0-28-17.65-53.205-37.867-79.488-17.493-22.74-37.45-46.474-52.447-72.315 15.077-1.478 32.417-4.93 47.9-9.576 10.422-3.125 19.897-6.74 27.412-11.075 3.758-2.168 7.058-4.49 9.81-7.48 2.753-2.99 5.192-7.065 5.192-12.065 0-11.75-6.934-21.71-16.332-30.554-9.398-8.846-21.883-17.077-36.203-25.26-27.3-15.6-61.207-30.86-91.815-45.994-.814-16.3-4.988-34.915-15.017-50.96C302.418 69.276 283 54.99 256 54.99z" />
                              </svg>
                              <span>{label}: {displayedGames}{hasMore ? '...' : ''}</span>
                            </div>
                          )
                        })()}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {venue.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-0.5 text-[10px] font-bold bg-[#F5F0E9] dark:bg-[#3A3A3A] text-[#8367C7] dark:text-[#8367C7] rounded-md border border-[#8367C7]/10 dark:border-[#8367C7]/20"
                        >
                          {tag}
                        </span>
                      ))}
              ...
              {filteredVenues.length === 0 && (
                <div 
                  ...
                  className="flex flex-col items-center text-center p-6 bg-white dark:bg-[#2D2D2D] border border-[#3A3A3A]/10 dark:border-[#F5F0E9]/10 rounded-2xl shadow-sm gap-4 animate-in fade-in duration-300"
                >
                  <div className="w-12 h-12 rounded-full bg-[#8367C7]/5 dark:bg-[#8367C7]/15 border border-[#8367C7]/15 dark:border-[#8367C7]/25 flex items-center justify-center text-[#8367C7]">
                    <svg
