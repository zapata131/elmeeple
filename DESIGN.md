...
* **Dedicated Yelp-style storefronts (`/venue/[slug]`):** Dynamic storefront profile pages resolve clean, SEO-friendly slugs (such as `/venue/orcs-stories`). They feature a high-fidelity dual-column layout on desktop (60% catalog grid, 40% reviews and vibe hub) and a stacked single-column layout on mobile, optimized for 100+ games.
...
* **Onboarding portal route protection & Step 1 session confirmation:** To prevent unauthorized guest registrations, the `/onboarding` portal is a client-side protected route. Unauthenticated guests are automatically redirected to `/login?callbackUrl=/onboarding`. Step 1 of the onboarding wizard requires zero typing and acts as a read-only account confirmation screen, rendering the user's name, email, brand-purple meeple SVG avatar, and an emerald-green "Cuenta vinculada" success badge, securing direct, frictionless owner-account linkage.
