...
* **Global Game Search on the Map (Milestone 5):** A dedicated search mode toggle below the homepage search bar allows users to switch between searching for venues ("Buscar Locales") and searching for specific games ("Buscar Juegos"). In game search mode, the map pins and sidebar list are filtered in real-time to show only venues that have the searched game in their synced catalog. Cards in the sidebar render a brand-purple meeple-badge indicating the matched game (e.g. "Tiene Scythe") without using emojis.
...
* **Global Game Search Toggle & Match Badges (Milestone 5):** The homepage search bar includes a toggle to switch between searching for venues and searching for games. If a game search is active, cards in the sidebar list render a custom game-matching badge styled in brand-purple background opacity (5%), thin borders (15%), and brand-purple text (e.g. "Tiene Scythe") with a custom inline vector SVG icon (no emojis), providing clear visual feedback on why the venue matched.
