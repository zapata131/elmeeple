...
* **Complexity badges and Spanish alternate name search:** Games in the storefront catalog grid and list views render a custom complexity/weight badge (e.g. `Peso: 2.3/5`) styled in brand Malva `#8367C7` with a 5% opacity background (`bg-[#8367C7]/5`), a 15% opacity border (`border-[#8367C7]/15`), and bold text. The storefront search filter and the homepage map game search match against both the primary game name and BGG alternate names (supporting Spanish search terms).
* **Visible map area search with "Search in this area" trigger:** Panning or zooming the map triggers a floating "Buscar en esta área" (Search in this area) button at the top center of the map (similar to Google Maps or Airbnb), allowing users to manually refresh the search within the new bounds. However, when the map pans due to a location search or geolocation trigger, the bounds filter automatically synchronizes without requiring a manual click.
* **Nearest-Venues Fallback:** If the active search bounds do not contain any matching venues, the application displays a prominent coral warning banner in the sidebar ("No hay locales en esta área. Mostrando los locales más cercanos:") and falls back to listing the 5 closest venues (calculated via the Haversine formula from the center of the viewport). These fallback results respect all other active filters (search queries, categories, and radius).
* **Marker Clustering:** If multiple venues or communities are situated in close proximity (based on the current zoom level), they are dynamically clustered on the client side into a single circular brand-purple badge showing the number of locales. If any venue in the cluster has upcoming events scheduled, a glowing turquesa ping animation is rendered on the badge. Hovering over a cluster displays a tooltip with the number of locales and events, and clicking it smoothly zooms and centers the map on that cluster.
...
* **Database (Supabase and PostgreSQL):** Core tables include:
  ...
  * `venue_games`: Idempotent bulk upsert of games linked to `venues` (`bgg_id`, `name`, `thumbnail`, player counts, playing time, complexity, alternate_names). Unique constraint on `(venue_id, bgg_id)`.
  * `reviews`: Community reviews with `rating` (1-5), `comment`, `vibe_tags` text arrays, and `user_email`.
  * `events`: Stores tournaments and activities published by verified owners, including `venue_id` (nullable, physical host venue), `organizer_venue_id` (not null, organizing venue/community), `title`, `game`, `description`, `date` (timestamptz), `entry_fee` (numeric), `max_participants` (integer), and `registration_url` (nullable, text).
...
6. **Dynamic event-siting coordinates resolution:**
   Rather than hardcoding community locations, we resolve them dynamically on the client side by parsing their upcoming events, finding the next event's physical host coordinates, and placing the community pin there. This keeps the map accurate and reflects where the community actually meets.
